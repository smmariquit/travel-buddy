...
class _FindSimilarPeopleScreenState extends State<FindSimilarPeopleScreen> {
  ...
  Future<void> _sendFriendRequest(AppUser user) async {
    try {
      ...
      if (!currentSentRequests.contains(user.uid)) {
        ...
        // Update local state
        setState(() {
          _currentUser = _currentUser!.copyWith(sentFriendRequests: currentSentRequests);
        });

        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Friend request sent to ${user.firstName}!", style: GoogleFonts.poppins())),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Error sending friend request: ${e.toString()}", style: GoogleFonts.poppins())),
      );
    }
  }
  ...
  Future<void> _cancelFriendRequest(AppUser user) async {
    try {
      ...
      if (currentSentRequests.contains(user.uid)) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Friend request canceled", style: GoogleFonts.poppins())),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Error canceling request: ${e.toString()}", style: GoogleFonts.poppins())),
      );
    }
  }
  ...
  Future<void> _acceptFriendRequest(AppUser user) async {
    try {
      // Get current data
      List<String> currentReceivedRequests = _currentUser!.receivedFriendRequests ?? [];
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Friend request from ${user.firstName} accepted!", style: GoogleFonts.poppins())),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Error accepting request: ${e.toString()}", style: GoogleFonts.poppins())),
      );
    }
  }
  ...
  Future<void> _rejectFriendRequest(AppUser user) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Friend request rejected", style: GoogleFonts.poppins())),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Error rejecting request: ${e.toString()}", style: GoogleFonts.poppins())),
      );
    }
  }
  ...
  void _showUserDetails(AppUser user) {
    ...
    final bool sentRequest = _currentUser!.sentFriendRequests?.contains(user.uid) ?? false;
    final bool receivedRequest = _currentUser!.receivedFriendRequests?.contains(user.uid) ?? false;
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildFriendActionButton(context, user, isFriend, sentRequest, receivedRequest),
                ],
              ),
            ),
          );
  ...
  Widget _buildFriendActionButton(BuildContext context, AppUser user, bool isFriend, bool sentRequest, bool receivedRequest) {
    if (isFriend) {
      // Already friends
      return SizedBox(
      ...
      return SizedBox(
        ...
        child: ElevatedButton.icon(
          ...
          label: Text("Cancel Request", style: GoogleFonts.poppins(color: Colors.white  )),
          onPressed: () {
...
class SimilarUserCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bool sentRequest = currentUser.sentFriendRequests?.contains(user.uid) ?? false;
    final bool receivedRequest = currentUser.receivedFriendRequests?.contains(user.uid) ?? false;
